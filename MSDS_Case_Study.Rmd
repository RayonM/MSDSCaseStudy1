...
# Step 1 - Classify all beers that are not Ale as "Non-Ale". 
beersNBreweriesClean$IPAorOther = "Non-Ale"

# Step 2 Classify all beers that are not Ale as "Other-Ale".
...
# Step 4 Knn.cv model. Find the ideal k number
newdataset = beersNBreweriesClean %>% filter(beersNBreweriesClean$IPAorOther !="Non-Ale" )

set.seed(6)
splitPerc = .75
trainIndices = sample(1:dim(newdataset)[1],round(splitPerc * dim(newdataset)[1]))

train = newdataset[trainIndices,]
test = newdataset[-trainIndices,]
...
{
  trainIndices = sample(1:dim(newdataset)[1],round(splitPercentage * dim(newdataset)[1]))
  train = newdataset[trainIndices,]
  test = newdataset[-trainIndices,]
