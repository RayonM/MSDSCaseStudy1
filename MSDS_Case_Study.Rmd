...
breweries_Count_State %>% ggplot(mapping = aes(x = State, y = Count)) +
  ...
  geom_text(aes(State, Count+2, label = Count), data = breweries_Count_State, col="blue")+
...
new_breweries
totals = dplyr::count(new_breweries, State)
...
#### Added label=count so it's easier to find when hovering over
#Functioning Code but it's not correct
heatMap = ggplot(map.df, aes(x=long,y=lat, label=count, group = group)) +
  geom_polygon(aes(fill= count))+
  #geom_text(aes(x=long, y=lat, label = map.df$count), col="black", cex=3)+
  scale_fill_gradientn(colours=rev(heat.colors(15)),na.value="black")+
  ggtitle("Breweries per State")+
  coord_map()+
  geom_path()+
  xlab("") + ylab("")+
