...
Good day Mr. CEO & CFO. Throughout this document, I've provided responses to your questions regarding the Exploratory Data Analysis (EDA) as well as highlighted some key findings that may pique your interest. The analysis was done using the source files of the Beers and Breweries data. 
...
# Load the breweries data from the breweries.csv file.
...
<<<<<<< HEAD
<<<<<<< HEAD
  geom_text(aes(State, ABV_Median+0.002, label = ABV_Median), data = ABV_Median_State, size=2.5)+ #Added labels to the chart
=======
  geom_text(aes(State, ABV_Median+0.002, label = ABV_Median), data = ABV_Median_State, size=2.3)+ 
>>>>>>> 5d8703a18cc2b263edb0d5be4eabaf7a087ec393
=======
  geom_text(aes(State, ABV_Median+0.002, label = ABV_Median), data = ABV_Median_State, size=2.8)+ 
>>>>>>> 0feab62d3dde6218579e18c293dcd1d51de7215f
  theme_tufte()+
...
beers_Breweries$IPA = beers_Breweries %>% filter(beers_Breweries$Style == "India Pale Ale"| beers_Breweries$Style == "IPA")

df = which(grepl("\\bAle\\b", x$Beer_Style))# Add border things or use ^ to get True "Ale" from the names.
df
Train = beersNbreweries[df, ]

#Step 1
beers_Breweries$IPAorOther = "Non-Ale"

#Step 2 find any beer with the word Ale in the style
beers_Breweries$IPAorOther[grepl("\\bAle\\b", beers_Breweries$Style,)] = "Other-Ale"

#Step 3 find the IPAs

#Step 4 Knn.cv model. Find the ideal 


summary(beers_Breweries$IPAorOther)
beers_Breweries$IPAorOther = as.character(beers_Breweries$IPAorOther)
...
c = beersNbreweries %>% ggplot(mapping = aes(x = ABV, y = IBU, position ="jitter", fill = Ounces, size = 1)) +
  geom_point()+
  theme_tufte()+
  xlab("ABV") + ylab("IBU")+
