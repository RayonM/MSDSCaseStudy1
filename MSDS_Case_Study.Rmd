...
#Compute the median the ABV (alcohol content)
ABV_Median_State = beersNbreweries %>% group_by(State) %>% summarize(ABV_State_median = median(ABV))
ABV_Median_State

#Plot a bar chart
ABV_Median_State %>% ggplot(mapping = aes(x= State, y = ABV_State_median )) +
...
IBU_Median_State = beersNbreweries %>% group_by(State) %>% summarize(IBU_State_median = median(IBU))
IBU_Median_State

#Plot a bar chart

  IBU_Median_State %>% ggplot(mapping = aes(x= State, y = IBU_State_median )) +
  geom_bar(stat = "identity")+
  theme_wsj()+
...
beersNbreweries %>% ggplot(mapping = aes(x = ABV, y = IBU, ABV ="jitter", stroke = 1)) +
